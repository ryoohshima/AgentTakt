...
## 設定ファイルの「読まれる/読まれない」判定（ユーザーの指摘より・2026-08-19）

- **状況**: ユーザーの `~/.claude/.mcp.json` に agenttakt が登録済みなのに `claude mcp list` に出ないことを根拠に、「そのファイルは読まれていない」と断定して報告した。ユーザーから「.mcp.json に書けば読んでくれるんじゃなかったっけ」と指摘され、`cd ~/.claude` して再実行したところ実際には読まれた（`.mcp.json` は cwd 依存のプロジェクトスコープ設定だったため、cwd が別なら出ないのが正しい挙動）。
- **ミス/原因**: 「ある条件下で観測されない」を「仕様として読まれない」に短絡した。設定の探索パスは cwd・スコープ・優先順位に依存するのに、条件を 1 つも振らずに 1 回の観測から仕様を断定した。ユーザーの記憶（正しかった）を訂正する側に回ったため、誤りの害が大きかった。
- **再発防止ルール**: 設定ファイルが「読まれない」と結論する前に、条件を変えて最低 1 回は再観測する（cwd を変える・別スコープに置く・プローブ値を書いて一覧に現れるか見る）。「出ない」は「無効」ではなく「その条件では出ない」までしか意味しない。仕様の断定は実証してからにせよ。ユーザーの記憶と観測が食い違うときは、ユーザー側が正しい前提で条件差を探す。
- **付随知識**: Claude Code の MCP 設定は user=`~/.claude.json` の `mcpServers` / project=**cwd の** `.mcp.json` / local=`~/.claude.json` の `projects.<path>.mcpServers` の 3 層。`~/.claude/settings.json` の `mcpServers` は無視される（プローブで実証済み）。`claude mcp add` は `timeout` を書けないため、長時間ブロックするツールでは手当てが要る。
